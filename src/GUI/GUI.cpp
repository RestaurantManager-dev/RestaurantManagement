...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	pDitem->clr = DrawingColors[(int)pOrd->getType()];
    OrderStatus order_status = pOrd->getStatus();
	GUI_REGION reg;
...
void GUI::AddToDrawingList(Cook* pC)
{
	DrawingItem *pDitem=new DrawingItem;
	pDitem->ID = pC->getID();
	pDitem->clr = DrawingColors[(int)pC->getType()];
